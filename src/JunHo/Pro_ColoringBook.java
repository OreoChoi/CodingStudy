...
public class Pro_ColoringBook {
    ...
    public static int[] solution(int M, int N,int[][] picture){
        ...
        for (int i = 0; i < picture.length; i++) {
            for (int j = 0; j < picture[i].length; j++) {
                ...
                if (lPx >= 0 && picture[i][lPx] == curPx)
                    graph.put(cIdx, picture[i].length * i + lPx);
                if (rPx < picture[i].length && picture[i][rPx] == curPx)
                    graph.put(cIdx,picture[i].length * i + rPx);
                if (tPx >= 0 && picture[tPx][j] == curPx)
                    graph.put(cIdx,picture[i].length * tPx + j);
                if (dPx < picture.length && picture[dPx][j] == curPx)
                    graph.put(cIdx,picture[i].length * dPx + j);
            }
        }

        List<Integer> apartCount = new ArrayList<>();
        for (int i = 0; i < graph.getSize(); i++) {
            ...
            apartCount.add(bfsList(i, visit));
            visited.add(visit);
        }

        int max = 0;
        for (int n : apartCount) {
            max = Math.max(max, n);
        }
